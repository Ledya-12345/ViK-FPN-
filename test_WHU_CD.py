...
import time
from train_WHU_CD import Supervision_Train, CDataset
import argparse
...
def get_args():
    ...
    arg("--weights_path", type=str, default="lightning_logs/WHU_CD/version_3/checkpoints/", help="Path to the directory containing the checkpoint.")
    arg("--test_weights_name", type=str, default="unetkan_cd-epoch=31-val_mIoU=0.8734", help="Name of the checkpoint file without extension. Ignored if --auto_best is set.")
    ...
    arg("--test_root", type=str, default="dataset/WHU_CD/test", help="Path to the test dataset.")
...
def main():
    ...
    if not os.path.isfile(checkpoint_path):
        raise FileNotFoundError(
            ...
            f"Either train a model first (see train_WHU_CD.py / README.md), download the "
    ...
    with torch.no_grad():
        ...
        for input in tqdm(test_loader):
            ...
            for i in range(raw_predictions.shape[0]):
                ...
                save_result(input['img1'][i].cpu(), input['img2'][i].cpu(), masks_true[i].cpu(), predictions[i].cpu(), f"outputsWHU_CD/result_{global_idx + i}.png")
                global_idx += 1
